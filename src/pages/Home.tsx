    ...
    >
      <header className="sticky top-0 z-40 bg-background/80 backdrop-blur-md border-b border-border">
        ...
        <div className="flex w-full overflow-x-auto scrollbar-hide">
          ...
          >
            <span className={cn("transition-colors", activeFeed === 'for-you' ? "text-foreground" : "text-muted-foreground")}>For You</span>
            {activeFeed === 'for-you' && (
              <motion.div layoutId="feedTab" className="absolute bottom-0 left-1/2 -translate-x-1/2 w-16 h-1 bg-primary rounded-full" />
            )}
          </button>
          <button 
            ...
            className="flex-1 min-w-[100px] py-4 font-bold transition-colors relative hover:bg-accent/50 text-sm md:text-base whitespace-nowrap"
          >
            <span className={cn("transition-colors", activeFeed === 'following' ? "text-foreground" : "text-muted-foreground")}>Following</span>
            {activeFeed === 'following' && (
              <motion.div layoutId="feedTab" className="absolute bottom-0 left-1/2 -translate-x-1/2 w-16 h-1 bg-primary rounded-full" />
            )}
          </button>
          <button 
            onClick={() => setActiveFeed('vibes')}
            className="flex-1 min-w-[100px] py-4 font-bold transition-colors relative hover:bg-accent/50 text-sm md:text-base flex items-center justify-center gap-1 whitespace-nowrap"
          >
            <span className={cn("transition-colors flex items-center gap-1", activeFeed === 'vibes' ? "text-foreground" : "text-muted-foreground")}>
              ...
              {displayedPosts.length > 0 ? (
                displayedPosts.map((post, idx) => (
                  <motion.div
                    ...
                    transition={{ delay: Math.min(idx * 0.05, 0.5), duration: 0.4, ease: "easeOut" }}
                  >
                    <PostCard post={post} />
                  </motion.div>
                ))
                ...
                >
                  ...
                  <p>{activeFeed === 'vibes' ? "No one has shared their vibe yet. Be the first!" : "Be the first to share something with the world!"}</p>
                </motion.div>
              )}
            </AnimatePresence>
          </div>
